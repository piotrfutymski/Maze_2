...
{
	glm::mat4 tmpM(1.0f);
	tmpM = glm::translate(tmpM, glm::vec3(0, 1.0, 0.0));
	tmpM = glm::rotate(tmpM, glm::radians(120.0f), glm::vec3(1.0, 0, 0));
	e.emplace_back(std::make_unique<Torch>(_shaders["base"].get(), _shaders["shadow"].get(), _shaders["light_src"].get(), _models["torch"].get(), _textures["cail"].get(), _textures["cail"].get(), _textures["cail"].get(), param, p, pos * tmpM));
}
